...
import com.dms.oauth.inf.Constants;
import com.dms.oauth.inf.OAuthService;
...
public class AuthorizeController {
    ...
    public Object authorize(
        ...
        try {
            ...
            //检查传入的客户端id是否正确
            CheckClientIdCmd cmd = new CheckClientIdCmd();
            cmd.setIdClient(oauthRequest.getClientId());
            if (!clientService.checkClientId(cmd)) {
            ...
            if(!subject.isAuthenticated()) {
                if(!login(subject, request)) {//登录失败时跳转到登陆页面
                    ...
                    model.addAttribute("client", clientService.load(loadClientCmd));
                    return "oauth2login";
                }
            }
